...
extension TextView {
    ...
    internal func resetBullet(range: inout NSRange, bullet: PianoBullet?) {
        ...
        case true:
            ...
            let resetRange = NSRange(location: location, length: length)
    ...
    internal func addBullet(range: inout NSRange, bullet: PianoBullet?) {
        ...
        let addRange = NSRange(location: uBullet.paraRange.location, length: uBullet.baselineIndex - uBullet.paraRange.location)
        let mutableAttrString = NSMutableAttributedString(attributedString: attributedText.attributedSubstring(from: addRange))

        switch uBullet.isOrdered {
        case true:
            let relativeNumRange = NSRange(location: uBullet.range.location - addRange.location, length: uBullet.range.length)
            ...
            let relativePunctuationRange = NSRange(location: relativeNumRange.lowerBound + nextNumberStr.count, length: 1)
    ...
    internal func deleteBullet(range: inout NSRange, bullet: PianoBullet?) {
        ...
        let deleteRange = NSRange(location: uBullet.paraRange.location, length: uBullet.baselineIndex - uBullet.paraRange.location)
    ...
    internal func adjust(range: inout NSRange, bullet: inout PianoBullet?) {
        ...
        if let adjustBullet = PianoBullet(type: .shortcut, text: self.text, selectedRange: NSRange(location: range.location, length: 0)) {
            bullet = adjustBullet
        }
    ...
    internal func transformTo(bullet: PianoBullet?) {
        ...
        case true:
            ...
            let puncRange = NSRange(location: bullet.baselineIndex - 2, length: 1)
    ...
    internal func adjustAfter(bullet: inout PianoBullet?) {
        ...
        while uBullet.paraRange.location + uBullet.paraRange.length < attributedText.length {
            let range = NSRange(location: uBullet.paraRange.location + uBullet.paraRange.length + 1, length: 0)
            ...
            textStorage.addAttributes(
                ...
                range: NSRange(location: adjustNextBullet.baselineIndex - 2, length: 1))
