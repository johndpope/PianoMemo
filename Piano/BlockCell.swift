...
extension BlockCell {
    ...
    private func set(content: String) {
        ...
        if let headerKey = HeaderKey(text: content, selectedRange: NSRange(location: 0, length: 0)) {
        ...
        } else if let bulletKey = PianoBullet(type: .key, text: content, selectedRange: NSRange(location: 0, length: 0)) {
            //버튼에 들어갈 텍스트 확보(유저에게 노출되는 걸 희망하지 않으므로 텍스트 컬러 클리어 색깔로 만들기
            ...
            if bulletKey.isOn {
                mutableAttrString.addAttributes(FormAttribute.strikeThroughAttr, range: NSRange(location: 0, length: mutableAttrString.length))
            }
    ...
    internal func saveToDataSource() {
        ...
        if let formStr = header,
            ...
            let bulletValue = PianoBullet(type: .value, text: formStr, selectedRange: NSRange(location: 0, length: 0)) {
...
extension BlockCell {
    ...
    internal func addCheckAttrIfNeeded() {
        ...
        let isOn = PianoBullet(type: .value, text: formButton.title(for: .normal) ?? "", selectedRange: NSRange(location: 0, length: 0))?.isOn ?? false
    ...
    internal func addHeaderAttrIfNeeded() {
        guard textView.attributedText.length != 0 else { return }
        //체크 유무에 따라 서식 입히기
        guard let headerKey = HeaderKey(text: headerButton.title(for: .normal) ?? "", selectedRange: NSRange(location: 0, length: 0)) else { return }
    ...
    internal func setCheckOffIfNeeded() {
        guard let text = formButton.title(for: .normal),
            let bulletValue = PianoBullet(type: .value, text: text, selectedRange: NSRange(location: 0, length: 0)),
    ...
    internal func revertForm() {
        ...
        if let header = headerButton.title(for: .normal),
            let headerKey = HeaderKey(text: header, selectedRange: NSRange(location: 0, length: 0)) {
            //1. 버튼 리셋시키고, 히든시킨다.
            headerButton.setTitle(nil, for: .normal)
            headerButton.isHidden = true
            //2. 텍스트뷰 앞에 키를 넣어준다.
    ...
    internal func convert(bulletShortcut: PianoBullet) {
        textView.replaceCharacters(in: NSRange(location: 0, length: bulletShortcut.baselineIndex), with: NSAttributedString(string: "", attributes: FormAttribute.defaultAttr))
...
//        textView.selectedRange.location -= bulletShortcut.baselineIndex
    ...
    internal func convert(headerKey: HeaderKey) {
        textView.textStorage.replaceCharacters(in: NSRange(location: 0, length: headerKey.baselineIndex), with: "")
